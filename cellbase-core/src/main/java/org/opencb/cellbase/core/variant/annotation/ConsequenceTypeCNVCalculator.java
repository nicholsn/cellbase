 ...
 */
public class ConsequenceTypeCNVCalculator extends ConsequenceTypeCalculator {
...
//            consequenceTypeList.add(new ConsequenceType(VariantAnnotationUtils.INTERGENIC_VARIANT));
    ...
    private void solveNonCodingNegativeTranscript() {
        ...
        String exonNumberString = null;
        if (variantStartExonNumber != null) {
            exonNumberString = variantStartExonNumber.toString();
        } else if (variantEndExonNumber != null) {
            exonNumberString = variantEndExonNumber.toString();
        }
        consequenceType.setExonOverlap(Collections.singletonList(new ExonOverlap(exonNumberString, null)));
...
//                }
                ...
                solveStopCodonNegativeCNV(transcriptSequence, cdnaCodingStart, cdnaVariantStart, cdnaVariantEnd);
            }
...
//                if (transcript.unconfirmedEnd() && (finalNtPhase != 2)) {
    ...
    private void solveStopCodonNegativeCNV(String transcriptSequence, int cdnaCodingStart,
                                           int cdnaVariantStart, int cdnaVariantEnd) {
...
//                }
                stopToSolve = false;  // Stop codon annotation will be solved in the line below.
                solveStopCodonPositiveCNV(transcriptSequence, cdnaCodingStart, cdnaVariantStart, cdnaVariantEnd);
            }
            if (cdnaVariantEnd >= (transcript.getCdnaCodingEnd() - finalNtPhase)) {
                if (finalNtPhase != 2) {
//                if (transcript.unconfirmedEnd() && (finalNtPhase != 2)) {
    ...
    private void solveStopCodonPositiveCNV(String transcriptSequence, int cdnaCodingStart, int cdnaVariantStart,
                                           int cdnaVariantEnd) {
        Integer variantPhaseShift1 = (cdnaVariantStart - cdnaCodingStart) % 3;
        Integer variantPhaseShift2 = (cdnaVariantEnd - cdnaCodingStart) % 3;
        int modifiedCodon1Start = cdnaVariantStart - variantPhaseShift1;
